...
define config.default_fullscreen = True # Приложение всегда запускается в полноэкранном режиме.
...
init -1: # Переключатели и Переменные.
    ...
    $ first_player = "" # Переменная , в которое вносится имя пользователя.
    $ gender_male = False
    $ firstname_list_male = ["Глеб", "Александр" , "Иван", "Ярослав", "Николай", "Тимур", "Михаил", "Константин", "Виталий", "Андрей"]
    $ lastname_list_male = ["Дроздов", "Носков", "Крылов", "Лихачёв", "Белов", "Иванов", "Якушев", "Морозов", "Лаврентьев", "Кондратьев"]
    $ firstname_list_female = ["Анастасия", "Валентина" , "Алина", "Валерия", "Мария", "Ирина", "Татьяна", "Эвелина", "Полина", "Дарья"]
    $ lastname_list_female = ["Александрова", "Петрова" , "Мухина", "Беляева", "Нестерова", "Орехова", "Кулакова", "Мышкина", "Морозова", "Миронова"]
...
label splashscreen: # Начальный сплешскрин.
    image muiv ="gui/splashscreen/muiv.svg"
...
label start:    # Тут начинается движение игры.
    $ achievements_delete() # очищаем список с ачивками.
    $ notify("Теперь в игре появились достижения и сообщения!", xalign=.5, xmaximum=250)
    play sound "content/audio/effects/achievement.wav"
